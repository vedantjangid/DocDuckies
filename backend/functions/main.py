...
def perform_calculations(data):
    ...
    # Current Ratio (assuming Capital includes Current Assets and Total-Liabilities includes Current Liabilities)
    if capital is not None and total_liabilities is not None:
        calculations['Current-Ratio'] = capital / total_liabilities

    # Quick Ratio (assuming Capital includes Cash + Receivables + Short-term Investments)
    if capital is not None and total_liabilities is not None:
        calculations['Quick-Ratio'] = capital / total_liabilities  # Adjust if you can separate components
    ...
    # Return on Equity (assuming Capital represents equity)
    if net_profit is not None and capital is not None:
        calculations['Return-on-Equity'] = net_profit / capital

    # Debt-to-Equity Ratio
    if total_liabilities is not None and capital is not None:
    ...
    if total_income is not None and total_assets is not None:
